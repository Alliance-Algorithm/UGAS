...
public:
    ...
    explicit ArmorIdentifier_V3(Types&&... args)
        : _blueIdentifier(parameters::BlueLightBarHue)
        , _redIdentifier(parameters::RedLightBarHue)
        , _numberIdentifier(std::forward<Types>(args)...) {}
    ...
    std::vector<ArmorPlate> Identify(const cv::Mat& img, ArmorColor targetColor) {
        ...
        for (const auto& contour : contours) {
            if (auto&& lightBarOpt = _solveToLightbar(img, contour, targetColor)) {
                ...
                if constexpr (debugCanvas.lightbar) {
                    ...
                    // line(debugCanvas.lightbar.GetMat(), lightBar.top, lightBar.bottom,
                    // COLOR_BLUE, 5);
        ...
        for (size_t i = 0; i < lightBarsSize; ++i) {
            ...
            for (size_t j = i + 1; j < lightBarsSize; ++j) { // 一些筛选条件
                ...
                if (fmax(Isize, Jsize) / fmin(Isize, Jsize) > parameters::maxArmorLightRatio)
                    continue;
                if (fabs(lightBars[i].angle - lightBars[j].angle) > parameters::maxdAngle)
                    continue;
                if (malposition(lightBars[i], lightBars[j]) > parameters::maxMalposition)
                    continue;
                cv::Point2f Jcenter = (lightBars[j].top + lightBars[j].bottom) / 2;
                if (fabs(Icenter.y - Jcenter.y) * 2 / (Isize + Jsize) > parameters::maxLightDy)
                    continue;
                float lightBarDis = P2PDis(Icenter, Jcenter) * 2 / (Isize + Jsize);
                if (lightBarDis > parameters::bigArmorDis)
                    continue;
...
private:
    ...
    std::optional<LightBar> _solveToLightbar(
        const cv::Mat& img, const std::vector<cv::Point>& contour, ArmorColor targetColor) {
        ...
        if (contourSize >= 5) {
            ...
            if (confidence > 0.45f) {
                ...
                if constexpr (debugCanvas.lightbar) {
                    ...
                    putText(
                        debugCanvas.lightbar.GetMat(), angle,
                        cv::Point2f(box.center.x + 10, box.center.y), 0, 0.5, COLOR_YELLOW);
                }
