...
bool NumberIdentifier_V1::Identify(const cv::Mat& imgGray, ArmorPlate& armor) {
    ...
    static const std::vector<Point2f> dst = {
        {-4.5, 10.125},
        {-4.5, 25.875},
        {40.5, 25.875},
        {40.5, 10.125}
    ...
    if constexpr (debugCanvas.armorNum) {
        ...
        // 绘制识别数字
        cv::putText(
            debugCanvas.armorNum.GetMat(), std::to_string(maxLoc.x),
            Point(drawPos.x - 12, drawPos.y + 13), 1, 2.5, COLOR_GREEN, 3);
        // 绘制识别置信度
        // auto maxValStr = std::to_string(maxVal); maxValStr.resize(6);
        // cv::putText(debugCanvas.armorNum.GetMat(), maxValStr, armor.points[2], 1, 1.5,
        // COLOR_GREEN, 1.5);
    }
