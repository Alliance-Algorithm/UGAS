        ...
        if (MV_OK == nRet) {
            ...
            if (stDeviceList.nDeviceNum == 1) {
                ...
                if (nullptr != userDefinedName && !IsSameDeviceName(pDeviceInfo, userDefinedName)) {
                    ...
                    LOG(WARNING) << "Name of the only camera does not equal to the name passed in: "
                                 << userDefinedName;
                }
                ...
                if (nullptr != userDefinedName) {
                    ...
                    LOG(ERROR) << stDeviceList.nDeviceNum
            ...
            if (nullptr != pDeviceInfo) {
                ...
                nRet = MV_CC_CreateHandleWithoutLog(&_handle, pDeviceInfo);
                if (MV_OK == nRet) {
                    ...
                    if (MV_OK == nRet) {
                        ...
                        if (pDeviceInfo->nTLayerType == MV_GIGE_DEVICE) {
                            ...
                            if (nPacketSize > 0) {
                                ...
                                if (MV_OK != nRet)
                                    LOG(WARNING) << "Warning: Failed to set packet Size. nRet ["
                                                 << nRet << "]";
                            } else
                                LOG(WARNING) << "Warning: Get invaild packet Size: " << nPacketSize;
                        }
                        ...
                        if (MV_OK == nRet) {
                            if (parameters::RotateCameraImage) {
                                ...
                                if (MV_OK != nRet)
                                    LOG(WARNING) << "Warning: Failed to set reverse x. nRet ["
                                                 << nRet << "]";

                                nRet = MV_CC_SetBoolValue(_handle, "ReverseY", true);
                                if (MV_OK != nRet)
                                    LOG(WARNING) << "Warning: Failed to set reverse y. nRet ["
                                                 << nRet << "]";
                            } else {
                            ...
                            nRet = MV_CC_SetEnumValue(
                                _handle, "ExposureAuto", MV_EXPOSURE_AUTO_MODE_OFF);
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set auto exposure. nRet ["
                                             << nRet << "]";

                            nRet = MV_CC_SetFloatValue(
                                _handle, "ExposureTime", parameters::CameraExposureTime.count());
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set exposure time. nRet ["
                                             << nRet << "]";

                            nRet = MV_CC_SetFloatValue(_handle, "Gain", parameters::CameraGain);
                            if (MV_OK != nRet)
                                LOG(WARNING)
                                    << "Warning: Failed to set gain. nRet [" << nRet << "]";

                            if (parameters::CameraDigitalShift != 0.0f) {
                                nRet = MV_CC_SetBoolValue(_handle, "DigitalShiftEnable", true);
                                if (MV_OK != nRet)
                                    LOG(WARNING)
                                        << "Warning: Failed to set digital shift enable. nRet ["
                                        << nRet << "]";

                                nRet = MV_CC_SetFloatValue(
                                    _handle, "DigitalShift", parameters::CameraDigitalShift);
                                if (MV_OK != nRet)
                                    LOG(WARNING) << "Warning: Failed to set digital shift. nRet ["
                                                 << nRet << "]";
                            }

                            nRet = MV_CC_SetBoolValue(_handle, "AcquisitionFrameRateEnable", false);
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set acquisition frame rate "
                                                "enable. nRet ["
                                             << nRet << "]";

                            nRet = MV_CC_SetBayerCvtQuality(_handle, 1);
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set bayer cvt quality. nRet ["
                                             << nRet << "]";
                            ...
                            if (MV_OK == nRet) {
                                ...
                                LOG(ERROR) << "Failed to start grabbing. nRet [" << nRet << ']';

                        } else
                            LOG(ERROR) << "Failed to set trigger Mode. nRet [" << nRet << ']';
                        MV_CC_CloseDevice(_handle);

                    } else
                        LOG(ERROR) << "Failed to open device. nRet [" << nRet << ']';
                    MV_CC_DestroyHandle(_handle);

                } else
                    LOG(ERROR) << "Failed to create handle. nRet [" << nRet << ']';
            }
        } else
            LOG(ERROR) << "Failed to enum Devices. nRet [" << nRet << ']';
    ...
    std::tuple<cv::Mat, TimeStamp> Read() override {
        ...
        if constexpr (debugCanvas.master) {
            if (debugCanvas.master.DebugFrameHandler.Paused && pConvertData != nullptr) {
                img = cv::Mat(
                    stConvertParam.nHeight, stConvertParam.nWidth, CV_8UC3,
        ...
        if (nRet == MV_OK) {
            if constexpr (false) {
                // 调用opencv进行Bayer2BGR
                // 效果不太好，但可以作为参考
                //                cv::Mat src = cv::Mat(stImageInfo.stFrameInfo.nHeight,
                ...
                //                default:
                //                    LOG(ERROR) << "exPixelType: " <<
                //                    stImageInfo.stFrameInfo.enPixelType;
                //                    throw_with_trace(std::runtime_error, "Unsupported pixel type!
                //                    Maybe not a RGB camera or be incorrectly configured.");
                //                };
            } else {
                ...
                if (pConvertData == nullptr) {
                    ...
                    stConvertParam.enDstPixelType =
                        PixelType_Gvsp_BGR8_Packed;                           // output pixel format
