                ...
                if (MV_OK == nRet) {
                    ...
                    if (MV_OK == nRet) {
                        ...
                        if (MV_OK == nRet) {
                            ...
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set auto exposure. nRet [" << nRet << "]";

                            nRet = MV_CC_SetFloatValue(_handle, "ExposureTime", false ? 3000 : 1000);
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set exposure time. nRet [" << nRet << "]";
                            ...
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set digital shift enable. nRet [" << nRet << "]";

                            nRet = MV_CC_SetFloatValue(_handle, "DigitalShift", 5.9993f);
                            if (MV_OK != nRet)
                                LOG(WARNING) << "Warning: Failed to set digital shift. nRet [" << nRet << "]";
...
public:
    ...
    std::tuple<cv::Mat, TimeStamp> Read() override {
        ...
        if (nRet == MV_OK) {
            ...
            // ע�⣺����cv::Mat��һ�����⹹�캯����Matָ���ͼ�����ݲ�����Mat�����������Զ�������������������ڴ�й¶
