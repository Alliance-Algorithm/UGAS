...
public:
    ...
    std::unique_ptr<TargetInterface> Update(
        const BuffPlate3d& buff, const std::chrono::steady_clock::time_point& timestamp,
        int64_t predict_duratioin) {
        if (!angle_array_.empty()
            && timestamp - last_update_ > std::chrono::milliseconds(predict_duratioin)) {
            time_array_.clear();
            angle_array_.clear();
        }
        ...
        if (angle_array_.empty()) {
            ...
            if (angle_array_.size() > 10) {
                ...
                //                cv::plot::Plot2d::create(time_array_, angle_array_);
                //                plot->render(plot_result);
                //                cv::Mat plot_result_test;
                //                cv::Ptr<cv::plot::Plot2d> plot_test =
                //                cv::plot::Plot2d::create(time_array_, angle_array_test);
...
private:
    ...
    static double GetMinimumAngleDiff(double a, double b) {
        ...
        if (diff < -parameters::Pi / 5)
            diff += 2 * parameters::Pi / 5;
        else if (diff > parameters::Pi / 5)
            diff -= 2 * parameters::Pi / 5;
        return diff;
    }
