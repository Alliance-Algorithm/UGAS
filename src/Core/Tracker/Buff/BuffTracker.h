...
public:
    ...
    std::unique_ptr<TargetInterface>
        Update(const BuffPlate3d& buff, const std::chrono::steady_clock::time_point& timestamp) {
        if (!angle_array_.empty() && timestamp - last_update_ > std::chrono::milliseconds(1000)) {
            time_array_.clear();
            angle_array_.clear();
        }
