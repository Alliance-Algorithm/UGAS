...
void Serial::SerialImpl::open() {
    ...
    if (fd_ == -1) {
        ...
        case EINTR:
            ...
            return;
        case ENFILE:
        case EMFILE: THROW(IOException, "Too many file handles open.");
        default: THROW(IOException, errno);
        }
    }
...
bool Serial::SerialImpl::waitReadable(uint32_t timeout) {
    ...
    if (!FD_ISSET(fd_, &readfds)) {
        THROW(
            IOException, "select reports ready to read, but our fd isn't"
...
size_t Serial::SerialImpl::read(uint8_t* buf, size_t size) {
    ...
    while (bytes_read < size) {
        ...
        uint32_t timeout =
            std::min(static_cast<uint32_t>(timeout_remaining_ms), timeout_.inter_byte_timeout);
...
size_t Serial::SerialImpl::write(const uint8_t* data, size_t length) {
    ...
    while (bytes_written < length) {
        ...
        if (r > 0) {
            ...
            // This shouldn't happen, if r > 0 our fd has to be in the list!
            THROW(
                IOException, "select reports ready to write, but our fd isn't"
                             " in the list, this shouldn't happen!");
        }
    }
