...
inline void TfBroadcast(
    const char* header, const char* child, const Eigen::Vector3d& translation,
...
inline void ArmorPlateBroadcast(
    const std::vector<ArmorPlate3d>& armors, const std::vector<int>& armor_ids) {
#if ENABLE_ROS
    if (auto node = node_.lock()) {
