...
double Trajectory_FEM::Analyze(double distance, double angle, double altitudeTarget, double& flyTime) {
	...
	Point2f speed(_com.speed * cos(angle / 180 * PI), _com.speed * sin(angle / 180 * PI));
	int cnt = 0;
...
void Trajectory_FEM::GetShotAngle(const Target& target, TimeStamp ImgTime, double& yaw, double& pitch) {
